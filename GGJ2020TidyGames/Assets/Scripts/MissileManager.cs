...
public class MissileManager : MonoBehaviour
{
    ...
    void Start()
    {
        for (int i = 0; i < (maxMissiles / 2); i++)
        {
            ...
            VFXManager.instance.AddParticleSystemToVFXList(bulletExplosion, "bulletExplode", 2);
        }
        ...
        if (colour == "red")
        {
            foreach (GameObject _missile in redMissiles)
            {
                if (!_missile.activeSelf)
                {
                    ...
                    VFXManager.instance.PlayParticleSystemFromVFXList(redSpawner, "fireBullet", true, true, (redTankBarrel.transform.up * 0.75f));
                    return true;
                }
            }
        }
        else if (colour == "blue")
        {
            foreach (GameObject _missile in blueMissiles)
            {
                if (!_missile.activeSelf)
                {
                    ...
                    _missile.SetActive(true);
                    VFXManager.instance.PlayParticleSystemFromVFXList(blueSpawner, "fireBullet", true, true, (blueTankBarrel.transform.up * 0.75f));
                    return true;
                }
            }
        }
